...
# 建立標籤字典，將多個標籤放入字典中
label_dict = {row['No.']: row[['Radius', 'Exo/endo', 'Near', 'A/P/X', 'Location']].values 
...
class TumorDataset(Dataset):
    ...
    def _load_data(self):
        ...
        for patient in sorted(os.listdir(self.root_dir)):
            ...
            # 將標籤轉換為 one-hot 編碼
            tumor_labels = self.label_dict[patient]
            tumor_labels_one_hot = self.multi_label_to_one_hot(tumor_labels)
            ...
            for scan_type in ['horizontal', 'vertical']:
                ...
                for img_file in sorted(glob.glob(os.path.join(scan_path, '*.png'))):
                    ...
                    targets.append(tumor_labels_one_hot)
    ...
    def __getitem__(self, idx):
        ...
        target = self.targets[idx].clone().detach().float()  # 確保標籤為 float32
    ...
    # 將每個標籤轉換為 one-hot 編碼
    def multi_label_to_one_hot(self, labels):
        one_hot = []
        for label in labels:
            # Radius, Exo/endo, Near, Location 每個有 3 類（1, 2, 3），所以要轉換成 3 長度的 one-hot
            one_hot.append(F.one_hot(torch.tensor(label - 1, dtype=torch.long), num_classes=3))
        return torch.cat(one_hot).float()  # 合併所有 one-hot
...
# 輸出層調整為多個輸出，每個輸出對應每個標籤的 one-hot 長度
model.fc = nn.Sequential(
    nn.Linear(model.fc.in_features, 128),
    nn.ReLU(),
    nn.Linear(128, 15)  # 5 個標籤，每個標籤轉換成 3 個 one-hot 編碼
)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model.to(device)

# 使用 BCEWithLogitsLoss 來處理多標籤任務
...
def evaluate_with_confusion(model, dataloader):
    ...
    with torch.no_grad():
        for images, targets in dataloader:
            images, targets = images.to(device), targets.to(device)
            outputs = torch.sigmoid(model(images))  # 使用 sigmoid 將輸出轉換為概率
            preds = (outputs > 0.5).float()  # 多標籤預測 (閾值為 0.5)
            all_preds.extend(preds.cpu().numpy())
            all_targets.extend(targets.cpu().numpy())

    cm = confusion_matrix(np.array(all_targets).flatten(), np.array(all_preds).flatten())
    accuracy = accuracy_score(np.array(all_targets).flatten(), np.array(all_preds).flatten())
